...
function load(projectFileName, globalSettingsFile, settings = {}) {
  return Promise.all([fse.readJson(globalSettingsFile).catch(err => {}), fse.readJson(projectFileName).catch(err => {})])
...
function save(projectFileName, globalSettingsFile, settings = {}) {
  ...
  const projectSettings = JSON.parse(
    JSON.stringify(
      Object.assign({}, settings, {
        variables: Array.isArray(settings.variables) ? settings.variables.join(',') : settings.variables
      })
    )
  );
  ...
  return Promise.all([
    fse.writeJson(globalSettingsFile, globalSettings, {spaces: 2}),
    Object.keys(projectSettings).length > 0 ? fse.writeJson(projectFileName, projectSettings, {spaces: 2}) : Promise.resolve()
  ])
    .catch(_ => settings)
    .then(() => settings);
}
