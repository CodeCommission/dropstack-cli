...
function login({username, password, url}) {
  ...
  .then(response => {
    ...
    return response.json().then(data => Object.assign({}, data, {version: response.headers.get('x-dropstack-version'), 'support-stateful': response.headers.get('x-dropstack-stateful') == 'true'}))
  });
}
