...
module.exports = config => {
  return {
    ...
    inputPasswordVerify: inputPasswordVerify
  };
};
...
function login({username, password, url}) {
  ...
  return fetch(`${url}/auth/login`, {
    ...
    return response.json().then(data => Object.assign({}, data, {version: response.headers.get('x-dropstack-version')}));
  });
}
...
function inputPasswordVerify(password) {
  if (password) return Promise.resolve({password});
